...
const char* hrt_stringus() {
    uint64_t duration = hrt_result();
    snprintf(hrt_str, HRT_MAX_STR_LEN, "%" PRIu64 "us", duration/1000);
    return hrt_str;
}

const char* hrt_stringms() {
    uint64_t duration = hrt_result();
    snprintf(hrt_str, HRT_MAX_STR_LEN, "%" PRIu64 "ms", duration/1000000);
    return hrt_str;
}
